...
with cel1:
    ...
    st.write(f"**{-buy_transactions['amount'].sum():.2f} €**") # negative because buys are negative amounts
with cel2:
...
allocation_by_asset = (
    buy_transactions.groupby(["name", "symbol"], as_index=False)
    .agg(
...
with col2:
    ...
    allocation_over_time = (
        buy_transactions.groupby("date", as_index=False)
        .agg(total_invested=("amount", "sum"))
    )
